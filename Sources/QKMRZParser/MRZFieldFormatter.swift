...
class MRZFieldFormatter {
    ...
    func format(_ string: String, as fieldType: MRZFieldType) -> Any? {
        ...
        case .documentType, .countryCode, .nationality, .personalNumber, .optionalData, .hash:
            return text(from: string)
        case .documentNumber:
            return correctDocumentNumber(string)
        }
    }
    
    func correctDocumentNumber(_ documentNumber: String) -> String {
        let start = documentNumber.index(documentNumber.startIndex, offsetBy: 2)
        let end = documentNumber.index(documentNumber.endIndex, offsetBy: -2)
        let range = start..<end

        let substring = documentNumber[range]
        let correctSubString = replaceLetters(in: String(substring))
        
        return documentNumber.replacingOccurrences(of: substring, with: correctSubString)
    }
